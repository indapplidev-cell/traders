...
def test_cached_wrapper_expected_candidate_count_matches_current_full_matrix() -> None:
    ...
    assert len(ML38_2_FV3_TUNING_CONFIG_IDS) == 24
    assert run_fv3_cached_tuning.DEFAULT_FULL_GRID_CONFIG_COUNT == 24
    assert run_fv3_cached_tuning.DEFAULT_EXPECTED_CANDIDATE_COUNT == expected
    assert run_fv3_cached_tuning.DEFAULT_EXPECTED_CANDIDATE_COUNT == 72
