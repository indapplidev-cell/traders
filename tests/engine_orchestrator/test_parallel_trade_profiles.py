...
def five_minute_config() -> OrchestratorConfig:
    return OrchestratorConfig(
        symbols=("BTCUSDT",), trade_profile_id="trade-5m-v2", primary_timeframe="5m",
...
def test_profiles_are_explicit_and_5m_is_the_only_runtime_default():
    ...
    assert TRADE_5M_V2_PROFILE.minimum_planned_rr == (
        TRADE_PARAMETERS.resolve_scalping_v2_parameter_set().parameters.geometry.minimum_planned_rr
    )
    ...
    assert TRADE_5M_V2_PROFILE.paper_command_creation_enabled is True
    assert TRADE_5M_V2_PROFILE.position_opening_enabled is True
    assert TRADE_5M_V2_PROFILE.trade_profile_id == "trade-5m-v2"
    assert TRADE_5M_V2_PROFILE.trade_mode == "SCALPING"
    assert TRADE_5M_V2_PROFILE.display_i18n_key == "trading.profile.trade_5m.title"
    assert TRADE_5M_V2_PROFILE.primary_timeframe == "5m"
    assert TRADE_5M_V2_PROFILE.entry_timeframes == ("1m", "5m")
    assert TRADE_5M_V2_PROFILE.context_timeframes == ("15m", "1h")
    assert dict(TRADE_5M_V2_PROFILE.market_data_windows) == {
        "1m": 60, "5m": 120, "15m": 64, "1h": 50,
    }
    assert TRADE_5M_V2_PROFILE.analysis_history_candles == 120
    assert TRADE_5M_V2_PROFILE.book_depth_limit == 100
    assert TRADE_5M_V2_PROFILE.microstructure_max_age_ms == 5_000
    assert dict(TRADE_5M_CONTEXT_MINIMUM_WINDOWS) == dict(
        TRADE_5M_V2_PROFILE.market_data_windows
    )
...
def test_closed_5m_detector_uses_profile_in_dedupe_identity():
    ...
    detector = ClosedWindowDetector(
        CandleRepo(), Store(), primary_timeframe="5m", trade_profile_id="trade-5m-v2"
    )
    windows = detector.get_unprocessed_closed_windows("BTCUSDT")
    assert len(windows) == 1
    assert seen[0][:3] == ("trade-5m-v2", "BTCUSDT", "5m")
...
def test_5m_production_runner_invokes_plan_stage_and_is_closed_only():
    ...
    assert result.trade_profile_id == "trade-5m-v2"
...
def test_profile_cursor_and_noneligible_materialization_are_isolated():
    ...
    result = PipelineResult(
        "BTCUSDT", "5m", BOUNDARY, trade_profile_id="trade-5m-v2",
        runtime_parameter_set_id=resolve_runtime_parameters("trade-5m-v2").parameter_set_id,
