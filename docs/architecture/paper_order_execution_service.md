...
| Repositories | `app/engine_paper/repositories.py` | `apply_entry_fill_and_open_position`, `apply_close_fill_and_close_position` | Locks, CAS, atomic fill/order/position/events/journal | yes | no | no | No graph fragments are persisted by the service |
...
simulation/repository outcomes, and source close boundary. It never contains
SQL, ORM objects, exceptions, traceback, candle payloads, environment data, or
credentials.
...
9. Reject a different active PAPER position; allow the same deterministic
   in-flight graph to reach repository replay resolution.
10. Call only `apply_entry_fill_and_open_position`.
11. Commit only through the UoW and return the committed graph summary.

The atomic repository owns fill insertion, order transition, position open,
event/journal persistence, locking, CAS, and active-position normalization.
...
resulting fill semantic tuple with the current bounded repository graph. This
permits an exact replay to return the current position graph even if later
legitimate lifecycle work changed that position, while conflicting fill
material fails with `IDEMPOTENCY_CONFLICT`.

Concurrent attempts converge at the existing row locks and deterministic fill
identity. One transaction creates the graph; another returns an idempotent,
stale, or controlled active-position outcome. No service path blindly retries a
mutation.
...
Matching, absent, conflicting, and unavailable lookup states remain distinct
typed outcomes.
