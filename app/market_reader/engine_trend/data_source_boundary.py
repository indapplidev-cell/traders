"""Storage-neutral candle provider boundary for engine_trend."""
...
class CandleDataBoundaryResult:
    ...
    engine_output: EngineTrendFacadeOutput
...
def _run_batch(batch: CandleDataBatch) -> EngineTrendFacadeOutput:
    symbol = batch.request.symbol or "UNKNOWN"
    interval = batch.request.interval or "UNKNOWN"
    return run_engine_trend(symbol, interval, batch.candles)


def run_engine_trend_from_batch(
    batch: CandleDataBatch,
...
def run_engine_trend_from_provider(
    ...
    *,
    ...
    try:
        ...
        return run_engine_trend_from_batch(batch)
