...
def build_support_resistance_zones(
    ...
    for point_type, zone_type in ((SwingPointType.LOW, ZoneType.SUPPORT), (SwingPointType.HIGH, ZoneType.RESISTANCE)):
        ordered = sorted((item for item in points if item.point_type is point_type), key=lambda item: item.price)
        ...
        for cluster in clusters:
            ...
            zones.append(SupportResistanceZone(
                zone_type, lower, upper, mid, len(cluster),
                tuple(sorted(item.index for item in cluster)), width,
...
def detect_trading_range(
    ...
    supports = sorted((item for item in zones if item.zone_type is ZoneType.SUPPORT), key=lambda item: (-item.touch_count, item.mid_price))
    resistances = sorted((item for item in zones if item.zone_type is ZoneType.RESISTANCE), key=lambda item: (-item.touch_count, -item.mid_price))
    ...
    inside_count = sum(lower <= item.close <= upper for item in candles)
    inside_ratio = inside_count / len(candles) if candles else 0.0
    ...
    return TradingRange(support, resistance, detected, lower, upper, midline, width, width_ratio, touches, inside_ratio)
...
def analyze_breakout_context(
    ...
    follow_window = later[:FOLLOW_THROUGH_LOOKAHEAD]
    follow_count = sum(beyond(item.close) for item in follow_window)
    confirmed = len(follow_window) == FOLLOW_THROUGH_LOOKAHEAD and follow_count == FOLLOW_THROUGH_LOOKAHEAD
    false_window = later[:FALSE_BREAKOUT_LOOKAHEAD]
    returned = any(lower <= item.close <= upper for item in false_window)
    status = BreakoutConfirmationStatus.FALSE_BREAKOUT if returned else BreakoutConfirmationStatus.CONFIRMED if confirmed else BreakoutConfirmationStatus.NO_FOLLOW_THROUGH
    ...
    if returned:
        ...
        items.extend((_evidence(false_code), _evidence("SCHWAGER_PRICE_RETURNED_TO_RANGE")))
    elif confirmed:
    ...
    return BreakoutContext(direction, status, index, boundary, close, abs(close - boundary) / abs(boundary) if boundary else 0.0, returned, follow_count, tuple(items))
...
def analyze_polarity_flip_context(
    ...
    for test_index in range(index + 1, min(len(candles), index + 1 + RETEST_LOOKAHEAD)):
        ...
        if held:
            ...
            return PolarityFlipContext(status, source, test_index, True, evidence)
...
def analyze_schwager_range_context(
    ...
    for zone in zones:
        ...
        items.extend((_evidence(identified, touches=zone.touch_count), _evidence(held, touches=zone.touch_count)))
