...
from app.market_reader.engine_trend.candle_morphology import CandleMorphology
from app.market_reader.engine_trend.schemas import BookSource, EngineTrendEvidence
...
def _evidence(code: str, candles: Iterable[CandleMorphology]) -> EngineTrendEvidence:
    items = tuple(candles)
...
def _append_with_cautions(
    output: list[EngineTrendEvidence],
    code: str,
...
def _detect_single(items: tuple[CandleMorphology, ...], output: list[EngineTrendEvidence]) -> None:
    for item in items:
        one = (item,)
        inverted = (
...
def _detect_pairs(items: tuple[CandleMorphology, ...], output: list[EngineTrendEvidence]) -> None:
...
def _detect_triples(items: tuple[CandleMorphology, ...], output: list[EngineTrendEvidence]) -> None:
...
def _detect_multi(items: tuple[CandleMorphology, ...], output: list[EngineTrendEvidence]) -> None:
...
def analyze_nison_pattern_catalog(
    ...
    unique: dict[tuple[str, tuple[str, ...]], EngineTrendEvidence] = {}
    for item in output:
