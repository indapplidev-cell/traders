"""Conservative Steve Nison candlestick evidence for engine_analysis."""
...
def _evidence(code: str, **metadata: Any) -> EngineAnalysisEvidence:
    description, contribution = _EVIDENCE[code]
    return EngineAnalysisEvidence(BookSource.NISON, code, description, contribution, metadata)


def _engine_heuristic_evidence(code: str, **metadata: Any) -> EngineAnalysisEvidence:
    """Create derived window evidence without attributing it to Nison."""

    description, contribution = _EVIDENCE[code]
    heuristic_metadata = {
        "evidence_origin": "ENGINE_ANALYSIS_HEURISTIC",
        "book_attribution": False,
        **metadata,
    }
    return EngineAnalysisEvidence(
        BookSource.ENGINE_ANALYSIS,
        code,
        description,
        contribution,
        heuristic_metadata,
...
def _unique_codes(items: tuple[EngineAnalysisEvidence, ...]) -> tuple[str, ...]:
...
class NisonCandleContext:
    ...
    evidence: tuple[EngineAnalysisEvidence, ...]
...
class NisonWindowContext:
    ...
    window_evidence: tuple[EngineAnalysisEvidence, ...]
    all_evidence: tuple[EngineAnalysisEvidence, ...]
    reason_codes: tuple[str, ...]
...
def _single_candle_evidence(morphology: CandleMorphology) -> tuple[EngineAnalysisEvidence, ...]:
    codes: list[str] = []
...
def analyze_nison_candle_context(candle: EngineAnalysisCandle) -> NisonCandleContext:
...
def _pair_evidence(previous: CandleMorphology, current: CandleMorphology) -> tuple[EngineAnalysisEvidence, ...]:
    codes: list[str] = []
    bullish_engulfing = (
...
def analyze_nison_window_context(
    ...
    window_items: list[EngineAnalysisEvidence] = []
