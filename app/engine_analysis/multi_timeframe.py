...
from app.engine_analysis.analysis_contract import interval_duration
from app.engine_analysis.engine import EngineAnalysisFacadeOutput, run_engine_analysis
...
class MultiTimeframeResult:
    ...
    outputs: dict[str, EngineAnalysisFacadeOutput]
    safety: EngineAnalysisSafety = EngineAnalysisSafety()

    def to_dict(self) -> dict[str, Any]:
        return {
            "service": "ENGINE_ANALYSIS_MULTI_TIMEFRAME",
            "contract_version": "engine_analysis_mtf_v1",
            "symbol": self.symbol,
...
def run_multi_timeframe_engine_analysis(
    symbol: str,
    candles_by_interval: Mapping[
        str, Sequence[EngineAnalysisCandle]
    ],
    *,
    decision_interval: str,
    ...
    outputs = {
        interval: run_engine_analysis(symbol, interval, tuple(candles_by_interval[interval]))
    ...
    directional = {EngineAnalysisRegime.UP, EngineAnalysisRegime.DOWN}
    ...
    if not higher:
        ...
        selected = EngineAnalysisRegime.UNKNOWN
    ...
    elif selected is EngineAnalysisRegime.UNKNOWN:
        alignment = TimeframeAlignment.INSUFFICIENT
