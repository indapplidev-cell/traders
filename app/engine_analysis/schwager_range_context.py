...
def _evidence(code: str, contribution_sign: int = 1, **metadata: Any) -> EngineAnalysisEvidence:
    description, contribution = _EVIDENCE[code]
    if code == "SCHWAGER_BREAKOUT_FOLLOW_THROUGH_CONFIRMED":
        contribution *= contribution_sign
    return EngineAnalysisEvidence(BookSource.SCHWAGER, code, description, contribution, metadata)
...
def analyze_breakout_context(
    ...
    beyond_closes: list[tuple[int, EngineAnalysisCandle]] = []
...
def analyze_schwager_range_context(
    ...
    items: list[EngineAnalysisEvidence] = []
    for zone in zones:
