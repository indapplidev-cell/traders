...
def build_book_evidence_bucket(
    source: BookSource,
    evidence: tuple[EngineAnalysisEvidence, ...] | list[EngineAnalysisEvidence],
) -> BookEvidenceBucket:
...
def _matrix_evidence(summary: ConfluenceConflictSummary) -> tuple[EngineAnalysisEvidence, ...]:
...
def analyze_book_evidence_matrix(
    ...
    nison_engine_evidence = tuple(
        item for item in nison.all_evidence if item.source is BookSource.ENGINE_ANALYSIS
    )
