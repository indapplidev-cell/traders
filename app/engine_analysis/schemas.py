"""Core schemas for the clean book-based engine_analysis module."""
...
class EngineAnalysisRegime(str, Enum):
    """Market state produced by engine_analysis."""
...
class TradeSignal(str, Enum):
    ...
    engine_analysis is not allowed to produce trading instructions.
    """
...
class EngineAnalysisCandle:
    """Single OHLCV candle used by engine_analysis."""
...
class BookEvidence:
    ...
    nison: tuple[EngineAnalysisEvidence, ...] = ()
    altunina: tuple[EngineAnalysisEvidence, ...] = ()
    schwager: tuple[EngineAnalysisEvidence, ...] = ()
    engine_analysis: tuple[EngineAnalysisEvidence, ...] = ()

    def all_items(self) -> tuple[EngineAnalysisEvidence, ...]:
        return self.nison + self.altunina + self.schwager + self.engine_analysis
    ...
    def to_dict(self) -> dict[str, list[dict[str, Any]]]:
        return {
            ...
            "engine_analysis": [item.to_dict() for item in self.engine_analysis],
        }
...
@dataclass(frozen=True)
class EngineAnalysisSafety:
    """Safety contract for engine_analysis outputs."""
    ...
    def __post_init__(self) -> None:
        if self.trade_signal is not TradeSignal.NOT_EVALUATED:
            raise ValueError("engine_analysis must not emit trading signals")
        if self.safe_for_runtime_trading:
            raise ValueError("engine_analysis must not be marked safe for runtime trading")
        if self.live_trading_connected:
            raise ValueError("engine_analysis must not connect live trading")
...
class EngineAnalysisResult:
    """Final result contract for engine_analysis."""
    ...
    period_end: str | None
    candle_count: int
    market_regime: EngineAnalysisRegime
    confidence: float
    ...
    warnings: tuple[str, ...] = ()
    errors: tuple[str, ...] = ()
    safety: EngineAnalysisSafety = field(default_factory=EngineAnalysisSafety)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "contract_version": "engine_analysis_result_v1",
            "symbol": self.symbol,
            ...
            "final_answer": {
                ...
                "is_conclusive": self.market_regime is not EngineAnalysisRegime.UNKNOWN,
                "is_abstention": self.market_regime is EngineAnalysisRegime.UNKNOWN,
                "meaning": (
                    "CONFIRMED_MARKET_HYPOTHESIS"
                    if self.market_regime is not EngineAnalysisRegime.UNKNOWN
