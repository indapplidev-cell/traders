...
from app.engine_analysis.input_period import EngineAnalysisInputPeriod
...
from app.engine_analysis.ohlc_integrity import OHLCIntegrityResult, validate_ohlc_integrity
...
class RegimeCandidateScores:
    ...
    flat_score: float
    unknown_score: float
    selected_regime: EngineAnalysisRegime
    confidence: float
...
class RegimeComposerOutput:
    input_period: EngineAnalysisInputPeriod | None
...
def score_regime_candidates(matrix: BookEvidenceMatrix, ohlc_integrity: OHLCIntegrityResult | None = None) -> RegimeCandidateScores:
    ...
    raw_ranked = sorted(((EngineAnalysisRegime.UP, up), (EngineAnalysisRegime.DOWN, down), (EngineAnalysisRegime.FLAT, flat), (EngineAnalysisRegime.UNKNOWN, unknown)), key=lambda item: item[1], reverse=True)
    up, down, flat, unknown = map(_clamp, raw_scores)
    invalid = ohlc_integrity is not None and not ohlc_integrity.is_valid
    candidates = ((EngineAnalysisRegime.UP, up), (EngineAnalysisRegime.DOWN, down), (EngineAnalysisRegime.FLAT, flat))
    ...
    if invalid:
        selected, confidence = EngineAnalysisRegime.UNKNOWN, 0.0
        ...
        selected, confidence = EngineAnalysisRegime.UNKNOWN, min(0.25, unknown)
        ...
        selected, confidence = EngineAnalysisRegime.UNKNOWN, min(0.35, max(unknown, winning))
        ...
        selected, confidence = EngineAnalysisRegime.UNKNOWN, min(0.35, unknown)
        codes.append("COMPOSER_HIGH_CONFLICT_UNKNOWN")
        fallback_reason = "COMPOSER_HIGH_CONFLICT_UNKNOWN"
    elif winning < MIN_REGIME_SCORE or (winning - ranked[1][1] < MIN_SCORE_MARGIN and not (selected is EngineAnalysisRegime.FLAT and clear_range)):
        selected, confidence = EngineAnalysisRegime.UNKNOWN, min(0.30, max(unknown, winning))
        ...
        if summary.conflict_level is EvidenceConflictLevel.HIGH:
            confidence = min(confidence, 0.45 if selected is EngineAnalysisRegime.FLAT else 0.35)
        elif summary.conflict_level is EvidenceConflictLevel.MEDIUM:
            confidence = min(confidence, 0.60)
    if selected is EngineAnalysisRegime.UNKNOWN:
    ...
    return RegimeCandidateScores(
        ...
        tuple((regime.value, score) for regime, score in sorted(((EngineAnalysisRegime.UP, up), (EngineAnalysisRegime.DOWN, down), (EngineAnalysisRegime.FLAT, flat), (EngineAnalysisRegime.UNKNOWN, unknown)), key=lambda item: item[1], reverse=True)),
        selected_before_fallback.value,
        selected is EngineAnalysisRegime.UNKNOWN and selected_before_fallback is not EngineAnalysisRegime.UNKNOWN,
        fallback_reason,
...
def _composer_status(scores: RegimeCandidateScores, matrix: BookEvidenceMatrix, integrity: OHLCIntegrityResult) -> tuple[RegimeComposerStatus, RegimeDecisionSource]:
    ...
    if matrix.confluence_conflict.conflict_level is EvidenceConflictLevel.HIGH and scores.selected_regime is EngineAnalysisRegime.UNKNOWN:
        return RegimeComposerStatus.HIGH_CONFLICT, RegimeDecisionSource.CONFLICT
    if scores.selected_regime is EngineAnalysisRegime.UNKNOWN:
        return RegimeComposerStatus.FALLBACK_UNKNOWN, RegimeDecisionSource.COMPOSER_SAFETY
    if scores.selected_regime is EngineAnalysisRegime.FLAT:
...
def compose_regime_from_matrix(symbol: str, interval: str, candles: tuple[EngineAnalysisCandle, ...] | list[EngineAnalysisCandle], matrix: BookEvidenceMatrix, ohlc_integrity: OHLCIntegrityResult | None = None, *, config: AnalysisWindowConfig | None = None, strict_timestamps: bool = False) -> RegimeComposerOutput:
    ...
    try:
        period = EngineAnalysisInputPeriod(symbol, interval, items)
    ...
    composer_evidence = tuple(EngineAnalysisEvidence(BookSource.ENGINE_ANALYSIS, code, "Regime composer market-reading context") for code in dict.fromkeys(composer_codes))
    matrix_engine = tuple(item for item in matrix.all_evidence if item.source is BookSource.ENGINE_ANALYSIS)
    ...
    result = EngineAnalysisResult(symbol or "UNKNOWN_SYMBOL", interval or "UNKNOWN_INTERVAL", items[0].timestamp if items else None, items[-1].timestamp if items else None, len(items), scores.selected_regime, scores.confidence, evidence, _decomposition(matrix, integrity), warnings, errors, EngineAnalysisSafety())
    selected_direction = {
        EngineAnalysisRegime.UP: HypothesisDirection.BULLISH,
        EngineAnalysisRegime.DOWN: HypothesisDirection.BEARISH,
        EngineAnalysisRegime.FLAT: HypothesisDirection.FLAT,
...
def _invalid_output(symbol: str, interval: str, candles: tuple[EngineAnalysisCandle, ...], integrity: OHLCIntegrityResult, error: str | None = None) -> RegimeComposerOutput:
    ...
    scores = RegimeCandidateScores(0.0, 0.0, 0.0, 1.0, EngineAnalysisRegime.UNKNOWN, 0.0, RegimeConfidenceLevel.NONE, codes)
    evidence = BookEvidence(engine_analysis=tuple(EngineAnalysisEvidence(BookSource.ENGINE_ANALYSIS, code, "Fail-closed composer context") for code in codes))
    result = EngineAnalysisResult(symbol or "UNKNOWN_SYMBOL", interval or "UNKNOWN_INTERVAL", candles[0].timestamp if candles else None, candles[-1].timestamp if candles else None, len(candles), EngineAnalysisRegime.UNKNOWN, 0.0, evidence, warnings=integrity.warnings, errors=errors)
...
def compose_engine_analysis_result(symbol: str, interval: str, candles: tuple[EngineAnalysisCandle, ...] | list[EngineAnalysisCandle], *, config: AnalysisWindowConfig | None = None, strict_timestamps: bool = False) -> RegimeComposerOutput:
    ...
    try:
        EngineAnalysisInputPeriod(symbol, interval, items)
    except (TypeError, ValueError) as exc:
