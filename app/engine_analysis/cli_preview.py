...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Preview an engine_analysis market reading")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        facade = run_engine_analysis_from_rows(symbol, interval, rows)
        if args.output:
            save_engine_analysis_json(facade.json_payload, args.output)
        if args.print_json:
        ...
        print(f"engine_analysis preview error: {exc}", file=sys.stderr)
        return 1
