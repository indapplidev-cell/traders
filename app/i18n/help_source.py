...
_TOPICS = (
    ...
    HelpTopic("Market", LocalizedText("Рынок", "Market"), (
        _s("purpose", "Назначение", "Purpose", "Для Сделки 15м используются GET /markets и detail. Для Скальпинга тот же экран читает profile-aware контекст exact10 из GET /trading/funnel?trade_profile=trade-5m-v1; 15m fallback запрещён.", "Trade 15m uses GET /markets and its detail. Scalping reads its exact10 profile-aware context from GET /trading/funnel?trade_profile=trade-5m-v1; 15m fallback is forbidden."),
