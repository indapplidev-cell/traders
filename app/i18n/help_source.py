...
_TOPICS = (
    ...
    HelpTopic("Trading Funnel", LocalizedText("Воронка поиска сделки", "Trading Funnel"), (
        ...
        _s("stages", "Этапы", "Stages",
           "Analysis → Structural Setup → Strategy eligible → Risk approved → PaperTradePlan → Quantity approved → Срок проверен → Одобрение создано → Допущен сейчас → Selector winner. Единица каждого счётчика — symbol. Счётчики этапов фиксируют исторический проход и поэтому монотонны; «Допущен сейчас» отдельно учитывает expiry и может стать false.",
           "Analysis → Structural Setup → Strategy eligible → Risk approved → PaperTradePlan → Quantity approved → Validity check passed → Approval created → Currently eligible → Selector winner. Every count unit is symbol. Stage counts record historical passage and remain monotonic; Currently eligible separately applies expiry and may become false."),
