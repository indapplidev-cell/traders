...
PAPER_CANARY_STATES = (
    ...
    "NO_ELIGIBLE_APPROVAL", "NO_TRADE_SIGNAL", "WAITING_FOR_ELIGIBLE_APPROVAL", "RUNNING", "POSITION_OPEN",
...
PAGE_CONTROLS = MappingProxyType({
    ...
    "PAPER Trading": ("refresh", "trade_selection", "next"),
    "Trade Control": ("refresh", "arm_or_start", "disable", "emergency_stop", "clear_emergency_stop"),
...
SHORTCUTS = MappingProxyType({
    "ru": ("Alt+О Dashboard", "Alt+Р Market", "Alt+П Trading Universe", "Alt+А Analysis",
           "Alt+С Setups", "Alt+В Trading Funnel", "Alt+И Incidents", "Alt+Т PAPER Trading", "Alt+К Trade Control", "Alt+Н Settings", "F5 active page"),
    "en": ("Alt+O Dashboard", "Alt+M Market", "Alt+P Trading Universe", "Alt+A Analysis",
           "Alt+C Setups", "Alt+F Trading Funnel", "Alt+I Incidents", "Alt+T PAPER Trading", "Alt+K Trade Control", "Alt+S Settings", "F5 active page"),
})

AUTO_REFRESH = MappingProxyType({
    "PAPER Trading": 10, "Trade Control": 10, "Dashboard": 30, "Market": 30, "Trading Universe": 30, "Trading Funnel": 10,
...
_GLOBAL = HelpTopic("global", LocalizedText("Обзор приложения", "Application overview"), (
    ...
    _s("refresh", "Обновление", "Refresh",
       "Опрос выполняется только для активной страницы: PAPER и Контроль сделки 10 с; Обзор, Рынок, Торговые пары, Анализ и Сценарии 30 с; Инциденты 60 с; Настройки и Help — выкл. Контроль сделки не загружается и не обновляется с экрана PAPER.",
       "Only the active page polls: PAPER and Trade Control 10s; Overview, Market, Trading Pairs, Analysis and Scenarios 30s; Incidents 60s; Settings and Help Off. Trade Control is neither loaded nor refreshed from the PAPER page."),
    _s("shortcuts", "Клавиатура", "Keyboard",
       "Alt+О — Обзор; Alt+Р — Рынок; Alt+П — Торговые пары; Alt+А — Анализ; Alt+С — Сценарии; Alt+И — Инциденты; Alt+Т — PAPER торговля; Alt+К — Контроль сделки; Alt+Н — Настройки. F5 обновляет активную страницу. Mutation hotkeys отсутствуют.",
       "Alt+O — Overview; Alt+M — Market; Alt+P — Trading Pairs; Alt+A — Analysis; Alt+C — Scenarios; Alt+I — Incidents; Alt+T — PAPER Trading; Alt+K — Trade Control; Alt+S — Settings. F5 refreshes the active page. There are no mutation hotkeys."),
))
...
_TOPICS = (
    ...
    HelpTopic("Market", LocalizedText("Рынок", "Market"), (
        _s("purpose", "Назначение", "Purpose", "Для Сделки 15м используются GET /markets и detail. Для Скальпинга тот же экран читает profile-aware контекст exact10 из GET /trading/funnel?trade_profile=trade-5m-v2; 15m fallback запрещён.", "Trade 15m uses GET /markets and its detail. Scalping reads its exact10 profile-aware context from GET /trading/funnel?trade_profile=trade-5m-v2; 15m fallback is forbidden."),
    ...
    HelpTopic("Trading Funnel", LocalizedText("Воронка поиска сделки", "Trading Funnel"), (
        ...
        _s("stages", "Этапы", "Stages",
           "Для Скальпинга сервер показывает цепочку Analysis → Structural Setup → Strategy → Risk compatibility → Geometry → Target → Net Cost → RR → Итоговый Risk → Portfolio → Final approval → PAPER plan. Risk compatibility — предварительная нерезервирующая проверка совместимости стратегии; Итоговый Risk — отдельный авторитетный допуск после геометрии, издержек и RR. Geometry проверяет причинные уровни и стоп, Net Cost — модельные издержки, RR — gross/net reward-to-risk, Portfolio — ограничения общего портфеля. N/A означает неприменимость или исторически не записанное поле; Недоступно означает, что источник не сериализовал факт. Для 15m сохраняется прежняя цепочка и downstream-поля показываются как N/A.",
           "For Scalping the server shows Analysis → Structural Setup → Strategy → Risk compatibility → Geometry → Target → Net Cost → RR → Authoritative Risk → Portfolio → Final approval → PAPER plan. Risk compatibility is the preliminary non-reserving strategy compatibility check; Authoritative Risk is a separate admission after geometry, costs, and RR. Geometry validates causal levels and stop placement, Net Cost evaluates modeled costs, RR evaluates gross/net reward-to-risk, and Portfolio represents account-wide constraints. N/A means not applicable or not historically recorded; Unavailable means the source did not serialize the fact. The existing 15m chain is preserved and downstream-only fields display N/A."),
        _s("metrics", "Отсев и конверсия", "Attrition and conversion",
           "Отсев и конверсия — presentation-only расчёты между соседними доступными этапами одного окна и одной единицы: отсев = предыдущий count − текущий count, конверсия = текущий / предыдущий. Discovery (Analysis → Structural Setup) показан отдельно от downstream bottleneck. Через Недоступно этапы не перепрыгиваются. 0 означает вычисленный ноль, N/A — неприменимо, Недоступно — сервер не сериализовал факт, а — — математически неопределимо.",
           "Attrition and conversion are presentation-only calculations between adjacent available stages in the same window and count unit: attrition = previous count − current count, conversion = current / previous. Discovery (Analysis → Structural Setup) is separate from the downstream bottleneck. Unavailable stages are never skipped. 0 is an evaluated zero, N/A is not applicable, Unavailable means the server did not serialize the fact, and — is mathematically undefined."),
        ...
        _s("candidate_math", "Математика кандидата", "Candidate math",
           "Для выбранного Scalping-символа detail использует наиболее информативную причинную запись bounded 4h-окна и показывает серверные Entry, Stop, Target, дистанции, Gross/Net/Required RR, издержки, expected edge, Risk %, quantity, notional и TTL. Клиент вычисляет только presentation-only RR margin = Net RR − Required RR. Plan, quantity approval, final approval, order, fill и position показаны как разные состояния; PAPER Plan не означает исполнение.",
           "For the selected Scalping symbol, detail uses the most informative causal record in the bounded 4h window and shows server-authored Entry, Stop, Target, distances, Gross/Net/Required RR, costs, expected edge, Risk %, quantity, notional, and TTL. The client computes only presentation-only RR margin = Net RR − Required RR. Plan, quantity approval, final approval, order, fill, and position are distinct states; a PAPER Plan does not imply execution."),
    ...
    HelpTopic("Analysis", LocalizedText("Анализ", "Analysis"), (
        _s("purpose", "Назначение", "Purpose", "Analysis всегда относится к выбранному профилю: 15m читает legacy analysis, Скальпинг — persisted 5m analysis context из profile-aware funnel DTO. Universe и порядок задаёт сервер.", "Analysis always belongs to the selected profile: 15m reads legacy analysis, while Scalping reads persisted 5m analysis context from the profile-aware funnel DTO. The server owns universe and order."),
        ...
        _s("controls", "Управление", "Controls", "Symbol selector и Refresh/F5; auto-refresh 30 с. Для Скальпинга selector использует только server universe; stale response другого symbol/profile отбрасывается.", "Symbol selector and Refresh/F5; auto-refresh 30s. Scalping uses only the server universe; stale responses for another symbol/profile are discarded."),
    )),
    HelpTopic("Setups", LocalizedText("Сценарии", "Scenarios"), (
        _s("purpose", "Назначение", "Purpose", "15m использует cursor-paged GET /setups. Скальпинг показывает server-authoritative structural scenario каждого exact10 symbol, включая rejected/no-setup и явную причину; это не ордера.", "15m uses cursor-paged GET /setups. Scalping shows the server-authoritative structural scenario for every exact10 symbol, including rejected/no-setup with an explicit reason; these are not orders."),
    ...
    HelpTopic("PAPER Trading", LocalizedText("PAPER Торговля", "PAPER Trading"), (
        _s("purpose", "Назначение", "Purpose", "Лёгкая read-only проекция PAPER: расчёт, bounded история, активная позиция, сверка и выбранный отчёт. Критерии и управление находятся на отдельном экране Контроль сделки.", "Lightweight PAPER read-only projection: account calculation, bounded history, active position, reconciliation, and the selected report. Criteria and controls live on the separate Trade Control screen."),
        ...
        _s("refresh", "Обновление/ошибки", "Refresh/errors", "Активная позиция и расчёт обновляются каждые 10 с; история и сверка — bounded/change-aware не чаще 60 с; отчёт — только по выбору сделки. Запросы Контроля сделки отсутствуют.", "Active position and account calculation refresh every 10s; history and reconciliation are bounded/change-aware at no more than once per 60s; report loads only after trade selection. Trade Control requests are absent."),
    )),
    HelpTopic("Trade Control", LocalizedText("Контроль сделки", "Trade Control"), (
        _s("purpose", "Назначение", "Purpose", "Отдельная lazy-страница для Критериев сделки и PAPER управления. До первого входа widgets, network requests и refresh отсутствуют.", "A separate lazy page for Trading Criteria and PAPER Control. Before first entry it has no widgets, network requests, or refresh."),
        _s("criteria", "Критерии сделки", "Trading criteria", "Read-only server-authoritative критерии без изменения strategy, risk, selector или trading thresholds.", "Read-only server-authoritative criteria without changing strategy, risk, selector, or trading thresholds."),
        _s("control", "PAPER управление", "PAPER control", "Использует существующие operator-control endpoints, readiness gates и safety contracts. LIVE остаётся выключен.", "Uses the existing operator-control endpoints, readiness gates, and safety contracts. LIVE remains disabled."),
        _s("lifecycle", "Lifecycle", "Lifecycle", "При входе выполняется одна начальная загрузка и запускается active-page refresh. При уходе scope отменяется; скрытый экран не обновляется, повторный вход не создаёт duplicate timer/widgets.", "Entry performs one initial load and starts active-page refresh. Leaving cancels the scope; the hidden screen does not refresh, and re-entry creates no duplicate timer or widgets."),
    )),
