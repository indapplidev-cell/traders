...
def book_l1_api_preview_command(
    symbol: str = typer.Option("BTCUSDT", "--symbol", help="Trading symbol, for example BTCUSDT."),
    interval: str = typer.Option("15m", "--interval", help="Candle interval, for example 15m."),
...
@cli.command("book-l1-15m-quality-review")
def book_l1_15m_quality_review_command(
    symbols: str | None = typer.Option(
        None,
        "--symbols",
        help="Comma-separated trading symbols, for example BTCUSDT,ETHUSDT,SOLUSDT.",
    ...
    interval: str = typer.Option("15m", "--interval", help="Only 15m is allowed for BOOK-L1-26."),
    window_size: int = typer.Option(300, "--window-size", min=1, help="Candles per timeline window."),
    window_count: int = typer.Option(4, "--window-count", min=2, max=6, help="Timeline window count from 2 to 6."),
    min_candles: int = typer.Option(50, "--min-candles", min=1, help="Minimum candles required per window."),
    output_json: Path = typer.Option(
        ...
        "--output-json",
        help="Stable JSON quality review evidence output path.",
    ),
    output_md: Path = typer.Option(
        Path("reports/book_l1/market_reader_15m_quality_review.md"),
        "--output-md",
        help="Stable Markdown quality review evidence output path.",
    ),
    strict: bool = typer.Option(
        False,
        "--strict",
        help="Return non-zero exit code when the quality review fails.",
    ),
    show_details: bool = typer.Option(
        False,
        "--show-details",
        help="Print per-symbol findings and reason codes.",
    ),
) -> None:
    """Run BOOK-L1-26 15m Market Reader quality review."""
    from app.data_audit.market_reader_15m_stabilization import (
        DefaultMarketReader15mStabilizationServices,
        MarketReader15mStabilizationConfig,
        MarketReader15mStabilizationRunner,
    ...
    selected_symbols = parse_quality_review_symbols(symbols, tuple(symbol or ()))
    review_config = MarketReader15mQualityReviewConfig(
        symbols=selected_symbols,
        interval=interval,
        window_size=window_size,
        window_count=window_count,
    ...
    if interval == "15m":
        stabilization_config = MarketReader15mStabilizationConfig(
            symbols=selected_symbols,
            interval=interval,
            window_size=window_size,
            window_count=window_count,
        ...
        with get_session() as session:
            stabilization_result = MarketReader15mStabilizationRunner(
                DefaultMarketReader15mStabilizationServices(CandleRepository(session))
            ).run(stabilization_config)
        if not stabilization_result.passed:
            typer.echo("BOOK-L1-26 15m Market Reader Quality Review")
            typer.echo("")
            typer.echo("Result: FAIL")
            typer.echo("Required 15m stabilization pipeline did not pass.")
            if strict:
                raise typer.Exit(1)

    result = MarketReader15mQualityReviewRunner().run(review_config)
    typer.echo(MarketReader15mQualityReviewFormatter().format(result, config=review_config))
    if strict and not result.passed:
        raise typer.Exit(1)


@cli.command("book-l1-l2-regime-alignment-review")
def book_l1_l2_regime_alignment_review_command(
    symbols: str | None = typer.Option(
        None,
        "--symbols",
        help="Comma-separated trading symbols, for example BTCUSDT,ETHUSDT,SOLUSDT.",
    ...
    interval: str = typer.Option("15m", "--interval", help="Only 15m is allowed for BOOK-L1-27."),
    ...
    output_json: Path = typer.Option(
        ...
        "--output-json",
        help="Stable JSON alignment review evidence output path.",
    ),
    output_md: Path = typer.Option(
        Path("reports/book_l1/l1_l2_regime_alignment_review.md"),
        "--output-md",
        help="Stable Markdown alignment review evidence output path.",
    ),
    strict: bool = typer.Option(
        False,
        "--strict",
        help="Return non-zero exit code only when the alignment review fails.",
    ),
    show_details: bool = typer.Option(
        False,
        "--show-details",
        help="Print per-symbol alignment details.",
    ),
) -> None:
    """Run BOOK-L1-27 L1-L2 regime alignment review."""
    ...
    selected_symbols = parse_regime_alignment_symbols(symbols, tuple(symbol or ()))
    review_config = RegimeAlignmentReviewConfig(
        symbols=selected_symbols,
        interval=interval,
    ...
    result = RegimeAlignmentReviewRunner().run(review_config)
    typer.echo(RegimeAlignmentReviewFormatter().format(result, config=review_config))
    if strict and not result.passed:
        raise typer.Exit(1)
...
def analyze_feature_regime_results(
    *,
    ...
    payload = {
        ...
        "candle_ta_context_feature_count": summary.get("candle_ta_context_feature_count"),
        "candle_ta_context_missing_reason": summary.get("candle_ta_context_missing_reason"),
        "book_setup_context_features_attached": summary.get("book_setup_context_features_attached"),
        "book_setup_context_feature_count": summary.get("book_setup_context_feature_count"),
        "book_setup_context_missing_reason": summary.get("book_setup_context_missing_reason"),
        "fv4_feature_count": summary.get("fv4_feature_count"),
        "nison_feature_count": summary.get("nison_feature_count"),
        "altunina_feature_count": summary.get("altunina_feature_count"),
        "path_context_feature_count": summary.get("path_context_feature_count"),
        "htf_context_feature_count": summary.get("htf_context_feature_count"),
        "missing_context_feature_count": summary.get("missing_context_feature_count"),
